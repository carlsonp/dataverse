...
if ! grep -q "Ddataverse.files.s3.type=s3" "/usr/local/payara6/glassfish/domains/domain1/config/domain.xml"; then
  # use : as delimiter
  sed -i "s:</java-config>:<jvm-options>-Ddataverse.files.s3.type=s3</jvm-options>\n</java-config>:" /usr/local/payara6/glassfish/domains/domain1/config/domain.xml
...
wait-for-it localhost:8080 -- tail -f /usr/local/payara6/glassfish/domains/domain1/logs/server.log